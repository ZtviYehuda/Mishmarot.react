...
export const DashboardFilters = ({
  ...
  }, [selectedDeptId, selectedSectionId, selectedTeamId, selectedStatusId, selectedServiceTypes, selectedAgeRange]);
  ...
  const handleApply = () => {
    ...
    if (stagedFilters.ageRange?.min) {
       onFilterChange("ageRange", stagedFilters.ageRange.max ? `${stagedFilters.ageRange.min}-${stagedFilters.ageRange.max}` : `${stagedFilters.ageRange.min}+`);
    } else {
  ...
  const FilterContent = (
    <div className="flex flex-col h-full bg-background overflow-hidden rounded-t-[2.5rem] sm:rounded-3xl">
      ...
      <div className="px-4 border-b border-border/30 shrink-0">
        <div id="filter-tabs" className="flex gap-6 overflow-x-auto no-scrollbar py-3">
          {[
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar max-h-[60vh]">
        ...
        {activeTab === "status" && (
          <div className="space-y-4">
            <Label className="text-xs font-black text-muted-foreground uppercase tracking-widest text-right block">סטטוסים</Label>
            <div className="flex flex-wrap gap-2.5">
              {allStatusTypes.map((type) => (
                <Button
                  key={type.id}
                  variant="ghost"
                  onClick={() => setStagedFilters({ ...stagedFilters, statusId: stagedFilters.statusId === type.id.toString() ? undefined : type.id.toString() })}
                  className={cn(
                ...
                >
                  <div className="w-2 h-2 rounded-full ml-2" style={{ backgroundColor: type.color }} />
                  {type.name}
                </Button>
              ))}
            </div>
        ...
        {activeTab === "service" && (
          <div className="space-y-4">
            <Label className="text-xs font-black text-muted-foreground uppercase tracking-widest text-right block">מעמד</Label>
            <div className="flex flex-wrap gap-2.5">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newTypes = isActive
                        ? stagedFilters.serviceTypes.filter((t) => t !== type.name)
                        : [...stagedFilters.serviceTypes, type.name];
                      setStagedFilters({ ...stagedFilters, serviceTypes: newTypes });
                    }}
                    className={cn(
                      "h-10 px-4 rounded-xl text-xs font-black transition-all border",
                      isActive
