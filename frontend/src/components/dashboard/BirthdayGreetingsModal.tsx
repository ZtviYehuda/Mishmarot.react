...
export const BirthdayGreetingsModal: React.FC<BirthdayGreetingsModalProps> = ({
  ...
  const handleSend = (emp: BirthdayEmployee) => {
    ...
    if (!sentList.includes(emp.id)) {
      ...
      markBirthdaySent(emp.id); // Notify backend
    }
  };
          ...
          <div className="bg-muted/30 p-4 rounded-xl border border-border/40">
            ...
            <div className="flex justify-between items-center mt-3">
              <p className="text-[9px] text-muted-foreground font-medium leading-loose">
                * <span className="text-primary font-bold">[שם]</span> יוחלף בשם
                השוטר.
                <br />*{" "}
                <span className="text-primary font-bold">[שם_המפקד]</span> יוחלף
                ב: <span className="font-bold underline">{commanderName}</span>
              </p>
              {!isEditing && (
                <button
                  onClick={() => setIsEditing(true)}
          ...
          <div className="space-y-3 max-h-56 overflow-y-auto pr-1 custom-scrollbar">
            <div className="flex justify-between items-center px-1">
              ...
              {sentList.length > 0 && (
                <button
                  onClick={handleReset}
                  className="flex items-center gap-1 text-[9px] font-bold text-muted-foreground hover:text-destructive transition-colors"
                >
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-bold text-muted-foreground">
                        {emp.phone_number || "אין מספר טלפון"}
                      </span>
                    </div>
