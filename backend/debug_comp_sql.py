...
def debug_comparison_sql():
    conn = get_db_connection()
    cur = conn.cursor()
    target_date = "2026-02-23"

    # Let's run the query for מחלקת מענה מבצעי (ID 1)
    # Using the exact same logic as in attendance_model.py
    ...
    res = cur.fetchone()
    print(f"Result for Dept 1: {res}")

    conn.close()


if __name__ == "__main__":
    debug_comparison_sql()
